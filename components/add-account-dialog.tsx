...
async function logHistory(supabase: ReturnType<typeof createClient>, accountId: string, eventType: string, description: string) {
  await supabase.from("account_history").insert({ account_id: accountId, event_type: eventType, description })
}
...
export function AddAccountDialog({ services, children }: AddAccountDialogProps) {
  ...
  const createNewAccount = async (data: FormData) => {
    ...
    if (!error && newAccount) {
      await logHistory(supabase, newAccount.id, "CUENTA_CREADA",
        `Cuenta creada. Vence el ${new Date(newAccount.expiration_date).toLocaleDateString('es-ES')}.`)
  ...
  const handleReactivate = async () => {
    ...
    await supabase.from("account_users").delete().eq("account_id", existingAccount.id!)
    ...
    if (!updateError) {
      await logHistory(supabase, existingAccount.id!, "CUENTA_REACTIVADA", "Cuenta archivada reactivada y usuarios anteriores eliminados.")
