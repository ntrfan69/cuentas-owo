...
export function AccountsTable({ accounts, customers, services }: AccountsTableProps) {
  ...
  const filteredAccounts = accounts.filter((account) => {
    ...
    const daysLeft2 = getDaysUntilExpiration(account.expiration_date)
    const isReallyExpired = daysLeft2 <= 0 && account.status !== "cancelled"
    const isReallyActive = daysLeft2 > 0 && account.status !== "cancelled"
    const matchesStatus = statusFilter === "all"
      || (statusFilter === "expired" && isReallyExpired)
      || (statusFilter === "active" && isReallyActive)
      || (statusFilter === "cancelled" && account.status === "cancelled")
                ...
                filteredAccounts.map((account) => {
                  ...
                  return (
                    <TableRow key={account.id}>
                      ...
                      <TableCell>
                        <Badge variant="secondary" className={getStatusBadgeColor(account.status === "cancelled" ? "cancelled" : daysLeft <= 0 ? "expired" : "active")}>
                          {account.status === "cancelled" ? "Cancelada" : daysLeft <= 0 ? "Vencida" : "Activa"}
                        </Badge>
                      </TableCell>
                      <TableCell>
