...
export function EditAccountDialog({ account, customers, services, children }: EditAccountDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!error) {
      ...
      if (formData.get("status") !== account.status) changes.push(`estado cambiado a ${formData.get("status")}`)
      if (formData.get("account_email") !== account.account_email) changes.push("email actualizado")
      if (formData.get("account_password")) changes.push("contraseña actualizada")
      if (formData.get("notes") !== account.notes) changes.push("notas actualizadas")
      const description = changes.length > 0 ? `Cuenta editada: ${changes.join(", ")}.` : "Cuenta editada."
      await supabase.from("account_history").insert({ account_id: account.id, event_type: "CUENTA_EDITADA", description })
