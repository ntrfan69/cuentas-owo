...
export function RenewAccountDialog({ account, children }: RenewAccountDialogProps) {
  ...
  useEffect(() => {
    if (open) setNewCost(account.total_cost || 0)
  ...
  const handleRenew = async () => {
    ...
    if (!error) {
      const newExpiration = new Date(account.expiration_date)
      newExpiration.setDate(newExpiration.getDate() + account.duration_days)
      await supabase.from("account_history").insert({
        account_id: account.id,
        event_type: "CUENTA_RENOVADA",
        description: `Cuenta renovada por ${account.duration_days} días. Nuevo vencimiento: ${newExpiration.toLocaleDateString('es-ES')}. Costo: $${newCost.toLocaleString('es-CL')}.`
      })
